      ...
      <div className='text-2xl mb-3' >
            ...
            return (
              <div className='py-4 border-t border-b text-gray-700  grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4' key={index}>
                  <div className='flex items-start gap-6'>
                    ...
                    <input onChange={(e)=>{e.target.value == '' || e.target.value == '0' ? null : updateQuantity(item.id,item.size,Number(e.target.value))}}  type="number"  className='border px-1 sm:px-2 py-1 max-w-10 sm:max-w-20 ' min={1} defaultValue={item.quantity} />
       ...
       <div className='flex justify-end my-20'>
           ...
           <div className='w-full text-end'>
            <button onClick={()=>{navigate('/placeorders')}} className='bg-black text-white px-8 py-3 my-8 text-sm'>PlaceOrder</button>
           </div>
            </div>
       </div>
    </div>
